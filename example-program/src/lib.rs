...
fn process_instruction<'a>(
    ...
    if account_buf[0] == SwitchboardAccountType::TYPE_VRF as u8 {
        // let aggregator: AggregatorState = get_aggregator(switchboard_feed_account)
        let randomness = switchboard_feed_account.get_verified_randomness()
                .map_err(|e| {
                msg!("Aggregator parse failed. Please double check the provided address.");
                return e;
            })?;
        out = String::from_utf8_lossy(&randomness)
    ...
    msg!("Current feed result is {}!", &out);
    Ok(())
}
